...
static uint16_t fast = 20;
static uint16_t slow = 15;
...
void drive_course(TIM_HandleTypeDef *htim3, I2C_HandleTypeDef *hi2c2, double speed) {
	...
	//segment 1
	reset_pose();
	drive_until(htim3, hi2c2, fast, dist_to_turn);
	...
	drive_until(htim3, hi2c2, fast, dist_to_turn+dist_of_tile);
	turn_degree(hi2c2, 90);
	...
	reset_pose();
	drive_until(htim3, hi2c2, slow, dist_to_turn+dist_of_tile);
	turn_degree(hi2c2, 90);
	...
	reset_pose();
	drive_pit(htim3, hi2c2, fast, dist_beside_wall+dist_of_tile);
	drive_until(htim3, hi2c2, slow, dist_to_turn+dist_of_tile+dist_of_tile);
	turn_degree(hi2c2, 90);
