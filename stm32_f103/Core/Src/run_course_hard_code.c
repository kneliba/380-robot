...
void drive_pit (TIM_HandleTypeDef *htim3, I2C_HandleTypeDef *hi2c2, double speed) {
	...
	while(!pitched_down){
		...
		drive_until(htim3, hi2c2, speed, dist_to_turn);
		if(curr_pitch < 0){
			pitched_down = true;
		}
	}
	...
	while(curr_pitch > 1){
		...
		drive_until(htim3, hi2c2, speed*1.2, dist_to_turn);
	}
}
...
void drive_course(TIM_HandleTypeDef *htim3, I2C_HandleTypeDef *hi2c2, double speed) {
	...
	//segment 4
	drive_pit(htim3, hi2c2, speed);
	drive_until(htim3, hi2c2, speed, dist_to_turn+dist_of_tile);
	turn_degree(hi2c2, 90);

	//segment 5 (2 single gravel)
	// driving slower for a shorter distance
	drive_until(htim3, hi2c2, speed*0.8, dist_to_turn+dist_of_tile);
	turn_degree(hi2c2, 90);
	...
	drive_pit(htim3, hi2c2, speed);
	drive_until(htim3, hi2c2, speed*0.8, dist_to_turn+dist_of_tile+dist_of_tile);
	turn_degree(hi2c2, 90);
	...
	//segment 10
	drive_until(htim3, hi2c2, speed, dist_to_turn+dist_of_tile+dist_of_tile);
	turn_degree(hi2c2, 90);
